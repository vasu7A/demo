...
(function(W,D) {
	...
	data.adSlot.style.marginBottom = "10px"
	data.tag= "<script async src='https://securepubads.g.doubleclick.net/tag/js/gpt.js'></script><div id='gpt-passback'><script>;window.googletag=window.googletag||{cmd:[]};googletag.cmd.push(function(){googletag.defineSlot('/21674279164/sl-standard-banners-3',[300,600],'gpt-passback').addService(googletag.pubads());googletag.enableServices();googletag.display('gpt-passback')});</script></div>";
	
	// data.tag="<div id='id3083'><script src='https://adserver.bidsxchange.com/?id=3083&adSlot=id3083'></script></div>";
	data.width=300;
	data.height=600;
	data.barHeight=20;
	data.adSlot.style.position='relative';
	data.adSlot.style.width=data.width+'px';
	data.adSlot.style.height=(getDim(data.adSlot).height+(data.barHeight*2))+'px';
	...
	data.main.style.height=(getDim(data.adSlot)+(data.barHeight*2))+'px';
	...
	data.topBar.innerHTML='Advertisement by Bidsxchange';

	data.bottomBar=D.createElement('div');
	data.bottomBar.id='topBar-'+data.id;
	data.bottomBar.style.fontSize='10pt';
	data.bottomBar.style.color='white';
	data.bottomBar.style.backgroundColor='black';
	data.bottomBar.style.width=(getDim(data.main).width-2)+'px';
	data.bottomBar.style.height=(data.barHeight-1)+'px';
	data.bottomBar.style.textAlign='center';
	data.bottomBar.innerHTML='Advertisement by Bidsxchange';
	...
	data.main.appendChild(data.bottomBar);
	...
	data.adIframe.style.left=getDim(data.sandbox).left+'px';
	data.adIframe.style.bottom='0px';
	...
	function set() {
		...
		console.log(getDim(data.adIframe));
		console.log(getDim(data.sandbox));
