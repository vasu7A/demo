...
var mobileTag = "<script async src='https://securepubads.g.doubleclick.net/tag/js/gpt.js'></script><div id='gpt-passback'><script>window.googletag = window.googletag || {cmd: []};googletag.cmd.push(function() {googletag.defineSlot('/21710144538/Brandigic-320x50', [320, 50], 'gpt-passback').addService(googletag.pubads());googletag.enableServices();googletag.display('gpt-passback');});</script></div>"
...
function insertAd(){
    ...
    if(deviceType == "desktop"){
        ...
        console.log("desktopFixed2");
        var div = document.createElement('div');
        div.style.display = "none"
        div.style.height = 0+"px"
        // div.style.width = parseInt(desktopAdDimensions[0]) + 40 + "px";
        div.style.width = "100%"
        ...
        window.top.onscroll = function(e){
        ...
        setTimeout(() => {
            var open = setInterval(() => {
                if(div.offsetHeight < (parseInt(desktopAdDimensions[1]))){
        ...
        if(parseInt(mobileAdDimensions[1]) < 150){
            scale = (parseInt(mobileAdDimensions[1]))/150
            height= height*scale
            width=width*scale
            console.log("Width:", width, "Height:",height, "scale:",scale);
        }
        ...
        setTimeout(() => {
            var open = setInterval(() => {
                if(div.offsetHeight < (parseInt(mobileAdDimensions[1]))){
                    div.style.display = "flex"
                    div.style.justifyContent = "center"
                    div.style.height = (div.offsetHeight+5)+"px"
                }else{
